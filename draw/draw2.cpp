...
#define ballcount 4 //ilosc wszystkich kul na planszy
#define R 50.0 //promien kazdej kuli
...
void paintCircles(HDC hdc)
{
	Graphics graphics(hdc);
	SolidBrush blackBrush(Color(255, 0, 0, 200));
	for (int i = 0; i < ballcount; i++)
		graphics.FillEllipse(&blackBrush, kula[i].x, kula[i].y, R*kula[i].waga, R*kula[i].waga);

}

void paintArm(HDC hdc)
{
	Graphics graphics(hdc);
	Pen pen(Color(255, 120, 15, 80), 4);
...
void MyOnPaint(HDC hdc, HWND hWnd)
{
	...
	// widocznie ta funkcja rz¹dzi chwytaniem kul
	for (int i = 0; i < ballcount; i++)
	{
		if (hold != i){
			float Rkula = kula[i].waga*R;
			if (kula[hold].y - Rkula >= 300 || kula[hold].y + Rkula > kula[i].y && kula[hold].x + Rkula >= kula[i].x + 15 && kula[hold].x + Rkula <= kula[i].x + 2 * Rkula - 15)
				KillTimer(hWnd, TMR_1);
		}
	}
	kula[hold].y = kula[hold].y + 4;
...
void draw_ARM(HDC hdc)
{
	...
	for (int i = 0; i<ballcount; i++)
	{
		float Rkula = R*kula[i].waga;
		if (armx2 > kula[i].x && armx2 <kula[i].x + Rkula && army2>kula[i].y  && army2 < kula[i].y + Rkula)
			{
				kula[i].x = armx2 - Rkula / 2;
				kula[i].y = army2 - Rkula / 2;
				holdk = 1;
			}
